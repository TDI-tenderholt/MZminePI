...
public class Veritomyx implements MassDetector
{
    ...
    {
		double noiseLevel = parameters.getParameter(VeritomyxParameters.noiseLevel).getValue();

		// Create a tree set of detected mzPeaks sorted by MZ in ascending order
		TreeSet<ScanMzDataPoint> mzPeaks = new TreeSet<ScanMzDataPoint>(
			new DataPointSorter(SortingProperty.MZ,
				SortingDirection.Ascending));

		// Create a tree set of candidate mzPeaks sorted by intensity in
		// descending order.
		TreeSet<ScanMzDataPoint> candidatePeaks = new TreeSet<ScanMzDataPoint>(
		...
		while (candidatePeaks.size() > 0)
		{
		    ...
		    ScanMzDataPoint currentCandidate = candidatePeaks.first();
	
		    // Calculate the exact mass and update value in current candidate (MzPeak)
		    double exactMz = calculateExactMass(currentCandidate);
		    currentCandidate.setMZ(exactMz);
	
		    // Add this candidate to the final tree set sorted by MZ and remove from tree set sorted by intensity
		    mzPeaks.add(currentCandidate);
		    candidatePeaks.remove(currentCandidate);
		}

		// Return an array of detected MzPeaks sorted by MZ
		return mzPeaks.toArray(new ScanMzDataPoint[0]);
    }
     ...
     * @return
     */
    private void getLocalMaxima(Scan scan, TreeSet<ScanMzDataPoint> candidatePeaks, double noiseLevel)
    {
		...
		for (int i = 0; i < scanDataPoints.length - 1; i++)
		{
		    ...
		    if ((!ascending) && (nextIsBigger || nextIsZero))
		    {
				...
				if (localMaximum.getIntensity() > noiseLevel)
				{
				    ...
				    candidatePeaks.add(new ScanMzDataPoint(localMaximum, rawDataPoints));
				}
     ...
     * @return double
     */
    private double calculateExactMass(ScanMzDataPoint currentCandidate)
	{
		/*
