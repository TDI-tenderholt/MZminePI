...
public class ScanExport implements MassDetector {
	...
	double noiseLevel = parameters.getParameter(
		ScanExportParameters.noiseLevel).getValue();
    ...
    private void getLocalMaxima(Scan scan,
	    TreeSet<ScanMzDataPoint> candidatePeaks, double noiseLevel) {
	...
	for (int i = 0; i < scanDataPoints.length - 1; i++) {
		...
		if (localMaximum.getIntensity() > noiseLevel) {
		    ...
		    candidatePeaks.add(new ScanMzDataPoint(localMaximum,
			    rawDataPoints));
		}
	...
	return ScanExportParameters.class;
    }
